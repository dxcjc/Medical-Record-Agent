...
import { createDatabaseSessionInvalidationRepository, createRedisSessionInvalidationRepository } from "./session-invalidation.repository";
describe("session invalidation repository adapters", () => {
    it("database adapter skeleton writes only token hash with TTL fields and exposes blocked readiness diagnostics", async () => {
        ...
        await expect(repository.findInvalidatedSession({ tokenHash, now: new Date("2026-06-09T09:30:00.000Z") })).resolves.toEqual({ tokenHash, invalidatedAt, expiresAt });
        await expect(repository.findInvalidatedSession({ tokenHash, now: new Date("2026-06-09T10:00:01.000Z") })).resolves.toBeNull();
        expect(repository.describe()).toEqual({
            ...
            readiness: {
                nextAction: "运行至少两个 API 实例共享 database session invalidation store 的登出/轮换失效 smoke。",
                requiredChecks: [
    ...
    it("redis adapter skeleton stores token hashes with PX TTL and never sends raw JWT/cookie values to Redis", async () => {
        ...
        await expect(repository.findInvalidatedSession({ tokenHash, now: new Date("2026-06-09T09:01:00.000Z") })).resolves.toEqual({ tokenHash });
        expect(repository.describe()).toEqual({
            provider: "redis",
...
//# sourceMappingURL=session-invalidation.repository.test.js.map
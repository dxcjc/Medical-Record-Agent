...
import { createApiServices, createInProcessJobQueueExecutor, createRedisJobQueueAdapter } from "./api-services";
...
async function waitForMockCall(mock) {
...
describe("api service composition", () => {
    ...
    it("broker job queue adapter contract 在无真实 broker 实现时 fail-fast blocked", async () => {
        ...
        expect(brokerAdapter.describe()).toEqual(expect.objectContaining({
            adapter: "broker",
            productionReady: false,
            blockedReason: "QUEUE_BROKER_ADAPTER_NOT_CONNECTED"
        }));
    ...
    it("Redis broker queue adapter skeleton 用 mock client 验证 lease/retry/dead-letter/heartbeat/幂等语义", async () => {
        ...
        expect(client.set).toHaveBeenCalledWith("medical-recognition-jobs:idem:recognition:job-redis-001", expect.any(String), { nx: true, px: 60000 });
        const firstLease = await queue.leaseNext();
        expect(firstLease).toEqual(expect.objectContaining({
        ...
        expect(client.pexpire).toHaveBeenCalledWith(expect.stringContaining(firstLease?.id ?? "missing-lease"), 30000);
        ...
        expect(secondLease).toEqual(expect.objectContaining({
            taskName: "recognition",
            attempt: 2,
            payload: {
                jobId: "job-redis-001"
    ...
    it("把 repositories、core orchestrator 和 provider registry 组合成 API services", async () => {
        ...
        expect(repositories.fileRepository.create).toHaveBeenCalledWith(expect.objectContaining({
        ...
        expect(repositories.jobsRepository.create).toHaveBeenCalledWith(expect.objectContaining({
            schemaKey: "lims-clinical-info",
            sourceFileId: "file-001",
            providerConfig: {
                ocrProviderKey: "http-ocr",
        ...
        expect(repositories.resultsRepository.upsertByJobId).toHaveBeenCalledWith(expect.objectContaining({
            jobId: "job-001",
            fields: [],
        ...
        expect(repositories.writebackRepository.create).toHaveBeenCalledWith(expect.objectContaining({
            jobId: "job-001",
            targetSystem: "lims",
            requestPayload: {
                clinicalInfo: {
        ...
        expect(repositories.writebackRepository.complete).toHaveBeenCalledWith("writeback-001", expect.objectContaining({
            status: "succeeded"
        }));
        ...
        expect(repositories.evaluationRepository.createRun).toHaveBeenCalledWith(expect.objectContaining({
            datasetId: "dataset-001",
        ...
        expect(repositories.evaluationRepository.createDataset).toHaveBeenCalledWith(expect.objectContaining({
            key: "lims-ci-v1",
            displayName: "LIMS 合成评估集",
            deidentified: true,
            metadata: {
        ...
        expect(repositories.evaluationRepository.addSample).toHaveBeenCalledWith(expect.objectContaining({
            datasetId: "dataset-001",
            externalId: "synthetic-001",
            metadata: {
                sourceType: "synthetic",
    ...
    it("默认创建识别任务时先入队返回 queued，再由后台执行器异步写入结果", async () => {
        ...
        expect(repositories.jobsRepository.updateStatus).toHaveBeenCalledWith(expect.objectContaining({
            id: "job-001",
            status: "completed",
    ...
    it("拒绝向未标记脱敏的数据集导入评估样本", async () => {
        ...
        await expect(services.evaluationService.importSamples({
    ...
    it("允许 synthetic 评估样本导入", async () => {
        ...
        await expect(evaluationService.importSamples({
            datasetId: "dataset-001",
            samples: [
                {
    ...
    it("创建评估 run 后执行 runner、持久化指标并完成 run", async () => {
        ...
        expect(repositories.evaluationRepository.markRunStarted).toHaveBeenCalledWith("run-001", new Date("2026-06-05T09:00:00.000Z"));
        expect(evaluationRunner.run).toHaveBeenCalledWith(expect.objectContaining({
            runId: "run-001",
        ...
        expect(repositories.evaluationRepository.upsertMetric).toHaveBeenCalledWith(expect.objectContaining({
            runId: "run-001",
            name: "field_accuracy",
            value: 1,
            unit: "ratio"
        ...
        expect(repositories.evaluationRepository.completeRun).toHaveBeenCalledWith("run-001", expect.objectContaining({
            status: "completed",
    ...
    it("上传文件字节时如果没有配置 storageProvider 会拒绝创建文件记录", async () => {
        ...
        await expect(services.fileService.createUpload({
            originalName: "record.pdf",
            mimeType: "application/pdf",
            checksumSha256: "sha-demo",
...
//# sourceMappingURL=api-services.test.js.map
...
const plaintextSecretKeyPattern = /^(api[-_]?key|open[-_]?ai[-_]?api[-_]?key|api[-_]?token|access[-_]?token|refresh[-_]?token|bearer[-_]?token|password|passphrase|secret|client[-_]?secret|secret[-_]?access[-_]?key|authorization|auth[-_]?header)$/i;
...
function findPlaintextSecretConfigPath(value, path = []) {
...
function isSecretDiagnosticsPath(path) {
    return path.some((segment) => segment === "secretDiagnostics");
}
function isSensitiveResponseKey(key, path) {
...
function maskSecretRefsRecord(value) {
    return Object.fromEntries(Object.entries(value).map(([secretKey, secretValue]) => [
        secretKey,
        {
...
function readProviderConfigString(record, keys) {
...
function hasSecretRef(record, key) {
    const value = record?.[key];
    return typeof value === "string" && value.trim().length > 0;
}
function addRequiredFieldError(errors, path, message) {
...
export function redactSensitiveRouteValue(value, path = []) {
    if (Array.isArray(value)) {
...
export function isRouteResponseObject(value) {
    return Boolean(value && typeof value === "object" && !Array.isArray(value));
}
export function assertRouteResponseObject(value, code) {
...
export function assertRouteResponseObjectList(values, code) {
...
export const createWebhookSubscriptionRouteInputSchema = z
    ...
    .strip();
//# sourceMappingURL=route-dtos.js.map
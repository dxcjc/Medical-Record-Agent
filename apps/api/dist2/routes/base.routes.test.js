...
import { registerResultRoutes } from "./results.routes";
...
function createRouteTools(permissions) {
...
async function waitForAuditRecord(record) {
...
describe("base route groups", () => {
    ...
    it("File API 上传时要求 job:create 并写入审计", async () => {
        ...
        expect(response.statusCode).toBe(200);
        expect(fileService.createUpload).toHaveBeenCalledWith(expect.objectContaining({
            originalName: "record.pdf"
        }));
        await waitForAuditRecord(tools.recordAudit);
        expect(tools.recordAudit).toHaveBeenCalledWith(expect.objectContaining({
            action: "file.upload",
            objectType: "file",
            result: "success"
        }));
    ...
    it("Feedback API 创建反馈时要求 feedback:create 并写入审计", async () => {
        ...
        expect(response.statusCode).toBe(200);
        expect(feedbackService.create).toHaveBeenCalledWith(expect.objectContaining({
            jobId: "job-001"
        }));
        await waitForAuditRecord(tools.recordAudit);
        expect(tools.recordAudit).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=base.routes.test.js.map
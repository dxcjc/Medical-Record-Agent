...
import { assertProductionQueueContract, buildSecretResolverContract, buildProductionQueueContract, buildProductionSessionInvalidationStoreContract, createEnvSecretResolver, createKmsSecretResolver, createProductionJobQueueAdapter, createSecretResolverFromEnv, createSecretManagerResolver, createProductionSessionInvalidationStore, createVaultSecretResolver, createProductionApiServices } from "./production-services";
function createPrismaClientStub() {
    return {
        ...
        providerConfig: {
            ...
            findUnique: vi.fn(async (_input) => null),
...
function createProductionEnvWithRealProvidersStub() {
...
function createProviderRuntimeFetchStub() {
    return vi.fn(async (_url, _init) => new Response(JSON.stringify({
        pages: [
            {
                page: 1,
                text: "合成病历：诊断：模拟诊断。",
...
function createSyntheticRecognitionDocument(documentId) {
    return {
        documentId,
...
async function drainProductionJobs(services) {
...
describe("production api services bootstrap", () => {
    ...
    it("无真实 OCR/LLM provider 时阻断识别创建而不是落回 mock", async () => {
        ...
        await expect(services.jobService.create({
            schemaKey: "lims-clinical-info",
            document: {
    ...
    it("装配真实 service 依赖，并让写回路径调用 LIMS adapter", async () => {
        ...
        await expect(services.providerService.checkProviderHealth({
            key: "local-storage",
            actor: {
                actorUserId: "user-001",
                authType: "jwt",
        ...
        await expect(services.providerService.setDefaultProvider({
            key: "mock-ocr",
            actor: {
                actorUserId: "user-001",
                authType: "jwt",
        ...
        expect(limsAdapter.execute).toHaveBeenCalledWith(expect.objectContaining({
            id: "writeback-001",
        ...
        expect(limsAdapter.execute).not.toHaveBeenCalledWith(expect.objectContaining({
            payload: {
                clinicalInfo: {
                    clinicalDiagnosis: "客户端伪造诊断"
                }
        ...
        expect(prisma.writebackAttempt.update).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                id: "writeback-001"
            },
    ...
    it("生产手工写回缺 RecognitionResult readyFields 时拒绝裸 fields，避免绕过服务端可信边界", async () => {
        ...
        const services = createProductionApiServices({
            ...
            prisma: prisma,
            limsWritebackAdapter: limsAdapter,
            providerRuntimeFetch: createProviderRuntimeFetchStub(),
            openAiResponsesClient: createOpenAiResponsesClientStub(),
            now: () => new Date("2026-06-05T09:00:00.000Z")
        });
        await expect(services.writebackService.execute({
            jobId: "job-001",
            confirmed: true,
            fields: [
                {
    ...
    it("生产识别编排把状态流转写入 Prisma 任务仓库", async () => {
        ...
        expect(recognitionJobUpdate).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                id: "job-001"
            },
            data: expect.objectContaining({
    ...
    it("生产高置信识别会自动调用 LIMS 写回 adapter 并持久化写回结果", async () => {
        ...
        expect(prisma.writebackAttempt.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                jobId: "job-001",
    ...
    it("生产识别任务会拒绝当前环境未配置的 providerConfig，避免静默落回默认 provider", async () => {
        ...
        await drainProductionJobs(services);
        expect(prisma.recognitionResult.upsert).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("生产 LangChain provider 未注入真实模型时会启动期失败，避免空候选伪成功", () => {
        ...
        expect(() => createProductionApiServices({
            env,
            prisma: createPrismaClientStub(),
            now: () => new Date("2026-06-05T09:00:00.000Z")
    ...
    it("生产 OpenAI Responses provider 使用注入的真实 SDK client 形状而不是占位 throw", async () => {
        ...
        expect(JSON.stringify(prisma.recognitionResult.upsert.mock.calls)).not.toContain("OPENAI_RESPONSES_CLIENT_NOT_INJECTED");
    });
    ...
    it("生产识别任务会使用数据库 active schema version 的写回路径", async () => {
        ...
        expect(resultPayload).toEqual(expect.objectContaining({
            status: "writeback_completed",
        ...
        expect(resultPayload).not.toEqual(expect.objectContaining({
            error: expect.objectContaining({
                code: "SCHEMA_CONFIG_NOT_AVAILABLE"
            })
        }));
    ...
    it("生产评估运行会执行 core runner、创建评估识别任务并持久化指标", async () => {
        ...
        expect(prisma.evaluationSample.findMany).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                datasetId: "dataset-001"
            },
            take: 1
        }));
        expect(prisma.recognitionJob.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                schemaKey: "lims-clinical-info",
        ...
        expect(prisma.evaluationMetric.upsert).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                runId_name: {
                    runId: "run-001",
                    name: "field_accuracy"
        ...
        expect(prisma.evaluationRun.update).toHaveBeenCalledWith(expect.objectContaining({
            where: {
                id: "run-001"
            },
            data: expect.objectContaining({
    ...
    it("生产评估运行会按 run schemaKey 解析 active schema，而不是固定使用内置 LIMS schema", async () => {
        ...
        expect(prisma.evaluationRun.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
                providerConfig: {
                    providerKey: "openai-responses-model"
                },
        ...
        expect(prisma.recognitionResult.upsert).not.toHaveBeenCalledWith(expect.objectContaining({
            create: expect.objectContaining({
                fields: [
                    expect.objectContaining({
                        fieldKey: "clinicalDiagnosis"
    ...
    it("生产文件上传会通过配置的存储 provider 写入字节", async () => {
        ...
        expect(storageProvider.put).toHaveBeenCalledWith(expect.objectContaining({
            key: "uploads/2026-06-05/record.pdf",
            body: Buffer.from("DEMO_PDF_BYTES"),
            contentType: "application/pdf"
        }));
        expect(prisma.storedFile.create).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({
    ...
    it("生产 HTTP OCR provider 健康检查会执行可注入的最小请求并隐藏 apiKey", async () => {
        ...
        expect(healthFetch).toHaveBeenCalledWith("http://ocr.internal/api/recognize", expect.objectContaining({
            method: "GET",
            headers: {
    ...
    it("生产 LIMS provider 健康检查会用 baseUrl 和 endpoint 执行 dry-run ping 且不泄露 token", async () => {
        ...
        expect(healthFetch).toHaveBeenCalledWith("http://localhost:8090/api/clinical-info/writeback", expect.objectContaining({
            method: "POST",
            headers: {
    ...
    it("secret resolver 工厂为 Vault/KMS/Secret Manager 预留 fail-fast 边界且不伪造真实接入", async () => {
        ...
        expect(buildSecretResolverContract({
        ...
        expect(resolver.contract).toEqual(expect.objectContaining({
            provider: "kms",
            productionReady: false,
    ...
    it("Vault/KMS/Secret Manager resolver skeleton 支持注入 mock client 且失败时只返回脱敏 blocked reason", async () => {
        const vaultClient = {
            readSecret: vi.fn(async (ref) => (ref === "kv/medical/ocr" ? "vault-ocr-token" : null))
        };
        const kmsClient = {
            decryptSecretRef: vi.fn(async (ref) => (ref === "ciphertext://medical/llm" ? "kms-llm-token" : null))
        };
        const secretManagerClient = {
            accessSecretVersion: vi.fn(async (ref) => (ref === "lims-api-token/latest" ? "sm-lims-token" : null))
        };
    ...
    it("Redis broker adapter factory 有 mock client 时返回 skeleton，但真实 broker smoke 前仍不标记生产通过", () => {
        ...
        expect(adapter?.describe()).toEqual(expect.objectContaining({
            adapter: "broker",
            brokerProvider: "redis",
            productionReady: false,
    ...
    it("生产服务装配可注入 Redis queue client，但 status 仍明确真实 broker smoke blocked", () => {
        ...
        expect(queue.describe()).toEqual(expect.objectContaining({
            adapter: "broker",
            brokerProvider: "redis",
            productionReady: false,
            blockedReason: "QUEUE_BROKER_SMOKE_NOT_RUN"
    ...
    it("保存的 HTTP OCR provider 会用 secretRefs 经 resolver 注入 Authorization", async () => {
        const prisma = createPrismaClientStub();
        const providerRuntimeFetch = vi.fn(async () => new Response(JSON.stringify({
            pages: [
                {
                    page: 1,
                    text: "临床诊断：肺腺癌。",
        ...
        await drainProductionJobs(services);
        expect(providerRuntimeFetch).toHaveBeenCalledWith("http://ocr.vendor.example/recognize", expect.objectContaining({
    ...
    it("保存的 HTTP LLM provider 会用 secretRefs 经 resolver 注入模型 apiKey", async () => {
        const prisma = createPrismaClientStub();
        const providerRuntimeFetch = vi.fn(async (url, init) => {
        ...
        await drainProductionJobs(services);
        expect(providerRuntimeFetch).toHaveBeenCalledWith("http://llm.vendor.example/v1/chat/completions", expect.objectContaining({
            headers: expect.objectContaining({
...
//# sourceMappingURL=production-services.test.js.map
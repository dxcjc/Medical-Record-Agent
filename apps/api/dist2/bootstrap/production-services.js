...
import { createJobOrchestrator, createLimsWritebackAdapter, createModelProvider, createOcrProvider, createOpenAiLangChainModel, createOpenAiResponsesClient, buildGenericJsonPayload, limsClinicalInfoSchema, runEvaluation, validateCoreSchemaDraftInput } from "@medical-record-agent/core";
import { createAuthService, createRepositorySessionInvalidationStore } from "../auth/auth.service";
import { PERMISSIONS } from "../auth/permissions";
import { createDatabaseSessionInvalidationRepository, createRedisSessionInvalidationRepository } from "../auth/session-invalidation.repository";
...
import { createApiServices, createRedisJobQueueAdapter } from "../services/api-services";
...
import { createLocalStorageProvider, createS3Client, createS3StorageProvider } from "../storage";
function readSecretResolverEnvValue(env, key) {
    const value = env[key];
    return value && value.trim().length > 0 ? value.trim() : undefined;
}
function readSecretResolverProvider(env) {
...
const secretResolverReadiness = {
    nextAction: "配置 SECRET_RESOLVER_PROVIDER=vault|kms|secret-manager 并接入真实 client/SDK，再重跑 provider health 与 production smoke。",
    requiredChecks: ["external-secret-resolution-smoke", "provider-health-secretRefs-smoke"]
};
export function buildSecretResolverContract(env = process.env) {
    ...
    const requiredKeys = provider === "vault"
...
function toExternalSecretResolution(provider, contract, value) {
    ...
    return {
        resolved: false,
        source: provider,
        reason: contract.blockedReason === "SECRET_RESOLVER_CONTRACT_INCOMPLETE"
            ? "SECRET_RESOLVER_CONTRACT_INCOMPLETE"
            : "SECRET_NOT_FOUND"
    };
}
function createExternalSecretResolver(provider, options = {}) {
    const env = {
...
export function createVaultSecretResolver(options = {}) {
...
export function createKmsSecretResolver(options = {}) {
    const resolverOptions = {};
    if (options.env !== undefined) {
        resolverOptions.env = options.env;
    }
...
export function createSecretManagerResolver(options = {}) {
    const resolverOptions = {};
    if (options.env !== undefined) {
        resolverOptions.env = options.env;
    }
...
export function createSecretResolverFromEnv(env = process.env) {
    const contract = buildSecretResolverContract(env);
...
export function createMockSecretResolver(secrets) {
    return {
        async resolve(ref) {
...
function readQueueEnvValue(env, key) {
    const value = env[key];
    return value && value.trim().length > 0 ? value.trim() : undefined;
}
function readQueueBrokerProvider(env) {
...
function readQueuePositiveInteger(env, key) {
...
function readSessionInvalidationEnvValue(env, key) {
    const value = env[key];
    return value && value.trim().length > 0 ? value.trim() : undefined;
}
function readSessionInvalidationPositiveInteger(env, key) {
...
function readSessionInvalidationStoreProvider(env) {
...
export function buildProductionQueueContract(env = process.env) {
    ...
    const mode = readQueueEnvValue(env, "QUEUE_MODE") === "broker" ? "broker" : "in-process";
    if (mode === "in-process") {
        return {
            mode,
            productionReady: false,
...
export function buildProductionSessionInvalidationStoreContract(env = process.env) {
    ...
    const mode = readSessionInvalidationEnvValue(env, "SESSION_INVALIDATION_STORE_MODE") === "repository"
        ? "repository"
        : "in-memory";
    if (mode === "in-memory") {
        return {
...
export function createProductionSessionInvalidationStore(options = {}) {
    const env = options.env ?? process.env;
...
export function assertProductionQueueContract(contract) {
...
export function createProductionJobQueueAdapter(options = {}) {
    const env = options.env ?? process.env;
...
function isInputJsonObject(value) {
    return Boolean(value && typeof value === "object" && !Array.isArray(value));
}
function readPayloadRecord(value) {
...
function readModelCandidates(value) {
...
function readReadyFields(value) {
    if (!Array.isArray(value)) {
        return [];
    }
...
function readReadyFieldsFromRecognitionResult(result) {
...
function isServerReadyWritebackJob(job, result, readyFields) {
...
function buildModelProvider(env, options = {}) {
    ...
    if (env.providers.llm.provider === "langchain") {
        ...
        const openAiLangChainConfig = apiKey
            ? {
                apiKey,
                model: env.providers.llm.model
            }
        ...
        const model = options.langChainModel ??
    ...
    if (env.providers.llm.provider === "openai-responses") {
        const client = options.openAiResponsesClient ??
...
function buildModelProviderOptions(options) {
...
function buildStorageProvider(env) {
...
function getConfiguredOcrProviderKey(env) {
    return env.providers.ocr.provider === "http" ? "http-ocr" : undefined;
}
function getConfiguredModelProviderKey(env) {
...
function createRealProviderNotConfiguredError(providerName) {
...
function createUnconfiguredOcrProvider() {
...
function createUnconfiguredModelProvider() {
...
function createConfigurationFailureResult(jobId, error) {
    if (!error) {
...
function createProviderConfigFailureResult(jobId) {
    const error = {
...
function createSchemaConfigFailureResult(jobId) {
    const error = {
...
function isUsableCoreSchemaDraft(value) {
...
async function findDefaultSavedProviderKey(input) {
    ...
    for (const provider of allProviders) {
        ...
        if (normalizedProvider.kind === input.expectedKind &&
...
async function resolveProductionProviderRuntime(input) {
    ...
    if (effectiveOcrProviderKey !== configuredOcrProviderKey) {
        ...
        providers.ocrProvider = provider;
    }
...
function createProviderConfigAwareOrchestrator(input) {
    return {
        ...
        async start(jobInput) {
            ...
            const orchestrator = schemaResolution.source === "builtin" && !providerSelection.providers
...
function createProductionRecognitionSchema(schema) {
    return {
        ...schema,
...
function buildStorageHealthProbeKey(now) {
    return `health-check/provider-health-${now().toISOString().replace(/[:.]/g, "-")}.txt`;
}
async function runStorageHealthProbe(storageProvider, now) {
...
function normalizeProviderConfigRecord(provider) {
    ...
    const config = isInputJsonObject(provider.config) ? provider.config : {};
    const providerMode = readSavedProviderMode(config);
    const status = typeof provider.status === "string" ? provider.status.toLowerCase() : undefined;
    const isMock = provider.isMock === true ||
...
function parseProviderKind(kind) {
...
function createProviderRegistry(env, storageProvider, now, providerHealthFetch, providerRepository, secretResolver) {
    const environmentProviders = [
        ...
        },
        {
            ...
            secretRefs: env.storage.driver === "s3" && (env.storage.s3.accessKeyId || env.storage.s3.secretAccessKey)
...
function createConfiguredLimsWritebackAdapter(env) {
...
function createAuditRecorder(auditRepository) {
    return (input) => {
        const payload = {
            action: input.action,
...
function isPersistableRecognitionStatus(status) {
    return [
        "queued",
        "running",
        "completed",
...
function createPrismaJobTransitionRepository(jobsRepository, now) {
    return {
        async recordTransition(transition) {
            ...
            await jobsRepository.updateStatus({
                ...
                ...(["completed", "partial_completed", "needs_review", "writeback_completed", "writeback_failed", "failed"].includes(transition.status)
                    ? { completedAt: now() }
                    : {}),
                trace: [
                    {
...
function toEvaluationDocumentInput(sample) {
    const inputRecord = readPayloadRecord(sample.input);
    const documentId = readString(inputRecord.documentId, readString(inputRecord.fileId, readString(inputRecord.externalId, sample.id)));
    const document = {
        documentId
    };
...
function mapRecognitionResultToEvaluationPrediction(result) {
    const validationByField = new Map(result.validation.fieldResults.map((field) => [field.fieldKey, field]));
    const candidates = result.validation.normalizedCandidates.length > 0 ? result.validation.normalizedCandidates : result.extraction?.candidates ?? [];
...
function collectEvaluationWarnings(result) {
...
function readEvaluationSchemaSelection(value) {
...
function withEvaluationSchemaSummary(result, schemaResolution) {
 ...
 */
export function createProductionApiServices(options) {
    ...
    const sessionInvalidationStoreOptions = {
    ...
    const productionWritebackExecutor = createProductionWritebackExecutor(options.env, writebackRepository, limsWritebackAdapter, now, jobsRepository, resultsRepository);
    const modelProviderOptions = buildModelProviderOptions(options);
    const createProductionRecognitionOrchestrator = (schema, providers = {}) => createJobOrchestrator({
    ...
    const builtinRecognitionOrchestrator = createProductionRecognitionOrchestrator(createProductionRecognitionSchema(limsClinicalInfoSchema));
    ...
    const createProductionEvaluationRecognitionOrchestrator = (schema, providers = {}) => createJobOrchestrator({
        repository: createPrismaJobTransitionRepository(jobsRepository, now),
        schema,
        ocrProvider: providers.ocrProvider ?? buildOcrProvider(options.env, modelProviderOptions),
    ...
    const services = createApiServices({
        ...
        providerRegistry: createProviderRegistry(options.env, storageProvider, now, providerHealthFetch, providerRepository, modelProviderOptions.secretResolver),
...
export function createProductionWritebackExecutor(env, repository = createWritebackRepository(new PrismaClient()), adapter = createConfiguredLimsWritebackAdapter(env), now = () => new Date(), jobsRepository, resultsRepository) {
    return async (input) => {
...
//# sourceMappingURL=production-services.js.map
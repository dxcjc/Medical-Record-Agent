...
async function waitForAuditRecord(record, expectedCalls) {
    const mock = record;
...
function createAuthService(permissions = [
    "schema:read",
    "job:create",
    "job:read",
    "feedback:create",
    ...
    return {
        ...
        isSessionTokenInvalidated: vi.fn((_token) => false),
        invalidateSessionToken: vi.fn(async (_token) => undefined),
...
function createServices(authService = createAuthService()) {
    return {
        authService,
        auditService: {
...
describe("api server routes", () => {
    ...
    it("支持从登录到任务、结果、反馈、写回、Provider 和评估的主流程", async () => {
        ...
        expect(services.writebackService.execute).toHaveBeenCalledWith(expect.objectContaining({
            jobId: "job-001",
            confirmed: true,
        ...
        expect(services.auditService.record).toHaveBeenCalledWith(expect.objectContaining({
            action: "file.upload",
            objectType: "file",
            result: "success"
        }));
    ...
    it("浏览器会话可通过 HttpOnly cookie 访问受保护路由，登出后同一 cookie 立即失效", async () => {
        ...
        authService.isSessionTokenInvalidated.mockImplementation((token) => invalidatedTokens.has(token));
        authService.invalidateSessionToken.mockImplementation(async (token) => {

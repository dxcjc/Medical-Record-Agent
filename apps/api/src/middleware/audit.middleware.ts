...
function resolveObjectId(request: FastifyRequest, descriptor: AuditDescriptor) {
...
function buildSafeMetadata(request: FastifyRequest, statusCode: number) {
  return {
 ...
 */
export function createAuditHooks(dependencies: AuditHooksDependencies) {
  const audit = (descriptor: AuditDescriptor): preHandlerHookHandler => {
    return async (request, reply) => {
      const writeAudit = (result: AuditResultValue) => {

...
function createContext(permissions: string[]): AuthContext {
  return {
...
async function createServer(schemaService: SchemaRouteService, permissions: string[]) {
...
describe("schema routes", () => {
  it("创建、更新、校验草稿时要求 schema:draft 权限并传入 actor", async () => {
    ...
    expect(schemaService.updateDraft).toHaveBeenCalledWith(
      expect.objectContaining({
        id: "draft-001",
        actor: context
      })
    );
    expect(schemaService.validateDraft).toHaveBeenCalledWith(
      expect.objectContaining({
        id: "draft-001",
        actor: context
      })

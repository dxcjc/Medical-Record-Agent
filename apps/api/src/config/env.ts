...
const rawEnvSchema = z.object({
  ...
  LLM_PROVIDER: z.enum(["mock", "langchain", "openai-compatible", "openai-responses"]).default("mock"),
...
const checkedEnvSchema = rawEnvSchema.superRefine((env, context) => {
  ...
  // mock provider 允许完全本地运行；真实 OCR provider 必须显式提供 endpoint，token 可按测试服务策略选填。
  ...
  // LangChain 是真实模型调用链路，必须至少提供一个模型访问密钥；mock provider 才允许无密钥本地运行。

import type { CoreSchemaDraft } from "./schemaValidator";
 ...
 * 
 * 设计原则：
 * 1. 字段与文档实际内容对齐，不臆造不存在的字段
 * 2. 勾选项用 boolean + rawText 双字段，OCR 无法判断时保留原文
 * 3. 手写体/低质量 OCR 字段标记 evidencePolicy 降低置信度阈值
 * 4. 检测项目是多选，用 list 类型
 */
export const tumorGeneTestSchema: CoreSchemaDraft = {
  key: "tumor-gene-test",
  label: "肿瘤基因检测申请单",
  version: "1.0.0",
  evidencePolicy: {
  ...
  fields: [
    ...
    },
    {
      ...
      comments: [
        ...
        "若日期明显不合法（如 20265714），保留原文并标记为疑似错误。",
      ],
      adapterHints: {
    ...
    // ========== 检测项目（核心字段） ==========
    {
      ...
      comments: [
        "从肺癌区域的勾选项中提取已选项目。",
        "选项包括：肿瘤9基因、肿瘤13基因、肺癌11基因、EGFR、肿瘤40基因、188基因、1021基因、肿瘤mrd（血液）、实体瘤40基因",
    ...
    },
    {
      ...
      comments: [
        "从消化道肿瘤区域的勾选项中提取已选项目。",
        "选项包括：肠癌3基因（+MSI）、MSI、UGT1A1、C-Kit、PDGFRA、肠癌4基因（+MSI）、胃癌18基因、肿瘤18基因、肿瘤40基因、林奇综合征",
        "OCR 无法区分 □ 和 ☑，保留所有选项文本。",
      ],
      adapterHints: {
    ...
    },
    {
      ...
      comments: [
        "从检测产品区域提取已选项目。",
        "可能包括：Onco1021-MRD、OncoD肿瘤用药基因检测、同源重组修复缺陷基因检测、OncoMD肿瘤疗效基因监测、脑胶质瘤基因检测、肿瘤临床超级外显子组基因检测、肿瘤融合基因检测、PD-L1 IHC检测、淋巴瘤基因检测",
        "部分项目已勾选（有√标记），部分未勾选。OCR 难以区分。",
      ],
      adapterHints: {
